...
export async function runCodeCommand(options) {
  try {
    ...
    if (!command) {
      ...
      try {
        // 1. 运行代码度量分析（这个命令通常能工作）
        console.log("Code Metrics:");
        try {
          const metricsResult = await codeCommandHandler.handleCommand("metrics", args);
          if (metricsResult && metricsResult.summary) {
            ...
            console.log(`  Functions: ${metricsResult.summary.totalFunctions || 0}`);
        ...
        try {
          ...
          if (depsResult && depsResult.dependencies) {
            console.log(`  Total Dependencies: ${depsResult.dependencies.total || 0}`);
            console.log(`  Direct Dependencies: ${depsResult.dependencies.direct || 0}`);
            console.log(`  Dev Dependencies: ${depsResult.dependencies.dev || 0}`);
          } else {
        ...
        console.log("\nQuick Issues Scan:");
        try {
          const issuesResult = await codeCommandHandler.handleCommand("issues", { ...args, limit: 10 });
          if (issuesResult && issuesResult.issues && issuesResult.issues.length > 0) {
            const highIssues = issuesResult.issues.filter(i => i.severity === "high").length;
            const mediumIssues = issuesResult.issues.filter(i => i.severity === "medium").length;
        ...
        console.log("  2. Run 'naturecode code issues --severity high' for critical issues");
        console.log("  3. Run 'naturecode code deps-security' for security checks");
        console.log("  4. Run 'naturecode code refactor' for refactoring suggestions");
        
        console.log("\nAvailable detailed commands:");
        const commands = codeCommandHandler.getAvailableCommands();
        commands.forEach((cmd) => {
          console.log(`  naturecode code ${cmd.command.padEnd(15)} - ${cmd.description}`);
        });
        
        console.log("\nQuick start examples:");
        console.log("  naturecode code analyze              # Full code analysis");
        console.log("  naturecode code issues --severity high  # Critical issues only");
        ...
        // 如果自动分析失败，回退到显示帮助
        console.log("\nAvailable code analysis commands:");
        const commands = codeCommandHandler.getAvailableCommands();
        commands.forEach((cmd) => {
        ...
        console.log("\n📦 Dependencies:");
        try {
          const depsResult = await codeCommandHandler.handleCommand(
            "deps",
            args,
          );
          if (depsResult && depsResult.dependencies) {
        ...
        console.log("\n🔍 Quick Issues Scan:");
        try {
          const issuesResult = await codeCommandHandler.handleCommand(
            "issues",
            { ...args, limit: 10 },
          );
          if (

...
if command -v naturecode &> /dev/null; then
    ...
    read -p "Update to v1.5.6? [Y/n]: " response
...
# 获取当前版本
CURRENT_VERSION=$(node -p "require('./package.json').version" 2>/dev/null || echo "1.5.6")
...
if command -v naturecode &> /dev/null; then
    ...
    echo "    naturecode model     # Configure AI model (12+ providers available)"
    echo "    naturecode delmodel  # Remove model configuration"
    ...
    echo "    naturecode --version # Show version"
    echo ""
    echo "  Advanced Features:"
    echo "    naturecode code      # Code analysis and generation"
    echo "    naturecode project   # Project management"
    echo "    naturecode plugin    # Plugin management"
    echo "    naturecode session   # Session management"
    echo "    naturecode review    # Code review"
    echo "    naturecode team      # Team collaboration"
    echo "    naturecode performance # Performance optimization"
    echo ""
    echo "  Start interactive session:"
    echo "    naturecode start     # Start AI-powered coding session"
else
    echo ""
...
echo "   git init"
echo "   git add ."
echo "   git commit -m "NatureCode v1.5.6""

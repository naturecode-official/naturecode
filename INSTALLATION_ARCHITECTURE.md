# NatureCode Installation Architecture

## 🏗️ 安装系统架构

### 层次结构

```
用户
  ↓
install.sh (主入口点)
  ↓
install-smart.sh (智能安装器)
  ├── Simple Mode (简单模式)
  └── Professional Mode (专业模式)
```

### 文件说明

#### 1. **主入口点** (`install.sh`)

- **大小**: 33行
- **作用**: 下载并启动智能安装器
- **用户命令**: `curl -fsSL https://raw.githubusercontent.com/naturecode-official/naturecode/main/install.sh | bash`
- **特点**: 轻量级，只负责下载

#### 2. **智能安装器** (`install-smart.sh`)

- **大小**: 434行
- **作用**: 询问用户安装模式，然后执行相应安装
- **特点**:
  - 询问用户选择安装模式
  - 根据选择调用不同安装逻辑
  - 统一的错误处理
  - 完整的安装后指导

#### 3. **简单模式** (集成在 `install-smart.sh` 中)

- **目标用户**: 普通用户
- **特点**:
  - 快速安静安装
  - 最小化输出
  - 自动决策
  - 推荐给大多数用户

#### 4. **专业模式** (集成在 `install-smart.sh` 中)

- **目标用户**: 开发者/高级用户
- **特点**:
  - 详细安装过程
  - 系统信息显示
  - 交互式提示
  - 更好的错误诊断

#### 5. **直接安装脚本** (备用)

- `install-simple.sh`: 直接简单安装（绕过模式选择）
- `install-universal.sh`: 通用安装器（询问 GitHub 用户名）
- `install-curl.sh`: 高级交互式安装器
- `install-local.sh`: 本地安装脚本

## 🎯 用户流程

### 标准流程 (推荐)

```
用户运行: curl .../install.sh | bash
        ↓
下载 install.sh
        ↓
下载 install-smart.sh
        ↓
询问: "Select installation mode:"
        ↓
用户选择: [1] Simple 或 [2] Professional
        ↓
执行相应安装
        ↓
显示安装后指导
```

### 简单模式流程

```
选择 Simple Mode
        ↓
显示横幅
        ↓
检查要求
        ↓
静默安装
        ↓
简单结果
```

### 专业模式流程

```
选择 Professional Mode
        ↓
显示横幅
        ↓
显示系统信息
        ↓
详细检查要求
        ↓
交互式安装
        ↓
详细结果和指导
```

## 🔧 技术实现

### 模式检测逻辑

```bash
# 在 install-smart.sh 中
ask_installation_mode() {
    echo "Select installation mode:"
    echo "1) Simple Mode"
    echo "2) Professional Mode"
    read -r choice

    case "${choice:-1}" in
        1) INSTALL_MODE="simple" ;;
        2) INSTALL_MODE="pro" ;;
        *) INSTALL_MODE="simple" ;;
    esac
}
```

### 双重日志系统

```bash
# 简单模式日志函数
log_info_simple() { echo -e "${BLUE}[INFO]${NC} $1"; }

# 专业模式日志函数
log_step() { echo -e "${MAGENTA}[STEP]${NC} $1"; }
log_info() { echo -e "${CYAN}[INFO]${NC} $1"; }
```

### 条件执行

```bash
# 根据模式执行不同逻辑
if [ "$INSTALL_MODE" = "simple" ]; then
    install_simple
else
    install_pro
fi
```

## 📊 模式对比表

| 特性             | Simple Mode | Professional Mode |
| ---------------- | ----------- | ----------------- |
| **输出详细程度** | 最小        | 详细              |
| **用户交互**     | 无          | 有                |
| **系统信息**     | 不显示      | 显示              |
| **错误诊断**     | 基本        | 详细              |
| **安装时间**     | 较快        | 稍慢              |
| **目标用户**     | 普通用户    | 开发者/高级用户   |
| **推荐度**       | 高          | 中                |

## 🚀 使用示例

### 标准安装 (推荐)

```bash
curl -fsSL https://raw.githubusercontent.com/naturecode-official/naturecode/main/install.sh | bash
```

### 直接简单安装 (绕过模式选择)

```bash
curl -fsSL https://raw.githubusercontent.com/naturecode-official/naturecode/main/install-simple.sh | bash
```

### 直接专业安装 (使用通用安装器)

```bash
curl -fsSL https://raw.githubusercontent.com/naturecode-official/naturecode/main/install-universal.sh | bash
```

## 🎨 用户体验

### 简单模式界面

```
╔══════════════════════════════════════════════════════════════════╗
║                   NatureCode Smart Installer                     ║
╚══════════════════════════════════════════════════════════════════╝

[INFO] Checking requirements...
[SUCCESS] Requirements OK
[INFO] Installing...
[SUCCESS] Successfully installed
```

### 专业模式界面

```
╔══════════════════════════════════════════════════════════════════╗
║                   NatureCode Smart Installer                     ║
╚══════════════════════════════════════════════════════════════════╝

[STEP] System Information
  OS: Darwin
  Node.js: v18.15.0

[STEP] Checking prerequisites
[INFO] Checking Node.js...
[SUCCESS] All prerequisites satisfied

[STEP] Installation Details
  Target version: v1.4.5.3
  Current version: not_installed

[STEP] Downloading NatureCode...
[INFO] Downloading from GitHub...
[SUCCESS] Dependencies installed
```

## 🔄 更新策略

### 版本更新

```bash
# 所有安装脚本都会:
1. 检测已安装版本
2. 如果版本不同，自动更新
3. 保留用户配置
```

### 脚本更新

- `install.sh`: 很少更新，只负责下载
- `install-smart.sh`: 包含主要逻辑，可独立更新
- 其他脚本: 作为备用选项

## 📁 文件清单

### 必需文件 (上传到 GitHub)

```
install.sh              # 主入口点 (33行)
install-smart.sh        # 智能安装器 (434行)
```

### 备用文件 (可选上传)

```
install-simple.sh       # 直接简单安装
install-universal.sh    # 通用安装器
install-curl.sh         # 高级安装器
install-local.sh        # 本地安装
```

### 文档文件

```
INSTALLATION_ARCHITECTURE.md  # 本文档
README_INSTALL.md            # 用户指南
CURL_INSTALL.md             # 详细安装指南
```

## 🎯 设计原则

### 1. 用户友好

- 第一句就问用户偏好
- 清晰的模式描述
- 默认选择简单模式

### 2. 模块化

- 主入口点轻量级
- 智能安装器功能完整
- 模式逻辑分离

### 3. 向后兼容

- 保留所有现有脚本
- 用户习惯不受影响
- 新增功能可选

### 4. 易于维护

- 统一错误处理
- 清晰的代码结构
- 详细的文档

## 🔍 测试验证

### 测试用例

1. **简单模式**: `echo "1" | ./install-smart.sh`
2. **专业模式**: `echo "2" | ./install-smart.sh`
3. **无效输入**: `echo "3" | ./install-smart.sh` (应默认简单模式)
4. **空输入**: `echo "" | ./install-smart.sh` (应默认简单模式)

### 验证步骤

```bash
# 1. 语法检查
bash -n install-smart.sh

# 2. 功能测试
./test-smart-installer.sh

# 3. 实际安装测试
./install-smart.sh
```

## 📈 优势总结

### 对用户

- ✅ **选择权**: 用户可以选择安装体验
- ✅ **简单性**: 普通用户得到最简单体验
- ✅ **透明性**: 高级用户看到详细过程
- ✅ **一致性**: 统一的安装后指导

### 对开发者

- ✅ **可维护性**: 清晰的架构
- ✅ **可扩展性**: 容易添加新功能
- ✅ **可测试性**: 模块化设计便于测试
- ✅ **文档完整**: 详细的架构说明

## 🚀 最终推荐

### 对于大多数用户

```bash
# 使用主安装器，选择简单模式
curl -fsSL https://raw.githubusercontent.com/naturecode-official/naturecode/main/install.sh | bash
```

### 对于开发者

```bash
# 使用主安装器，选择专业模式
curl -fsSL https://raw.githubusercontent.com/naturecode-official/naturecode/main/install.sh | bash
# 或直接使用通用安装器
curl -fsSL https://raw.githubusercontent.com/naturecode-official/naturecode/main/install-universal.sh | bash
```

这个架构提供了最佳的平衡：简单用户得到简单体验，高级用户得到详细控制，所有用户都从统一的安装系统中受益。
